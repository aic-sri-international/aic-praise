...
public class ModelVisitor extends RuleBaseVisitor<Expression> {
	...
	public Expression visitModel(RuleParser.ModelContext ctx) {
		...
		Expression result = Expressions
				.make("model", expressions(ctx.elements));
		return result;
	}
	...
	public Expression visitAtomic_rule(RuleParser.Atomic_ruleContext ctx) {
		...
		Expression result = Expressions.make(RuleConverter.FUNCTOR_ATOMIC_RULE,
				formula, potential);
	...
	public Expression visitProlog_rule(RuleParser.Prolog_ruleContext ctx) {
		...
		if (rhs != null) {
			result = Expressions.make(RuleConverter.FUNCTOR_PROLOG_RULE, potential, head, rhs);
		}
		else {
			result = Expressions.make(RuleConverter.FUNCTOR_PROLOG_RULE, potential, head);
		}
		return result;
	}
	...
	public Expression visitProbability_notation_rule(
		...
		Expression result = Expressions.make(RuleConverter.FUNCTOR_STANDARD_PROB_RULE, formula1, formula2, potential);
		return result;
	}
	...
	public Expression visitCausal_effect_rule(
		...
		Expression result = Expressions.make(RuleConverter.FUNCTOR_CAUSAL_RULE, cause, effect);
		return result;
	}
	...
	public Expression visitConditional_rule(
		...
		Expression result = null;
		if (elseRule != null) {
			result = Expressions.make(RuleConverter.FUNCTOR_CONDITIONAL_RULE, condition, thenRule, elseRule);
		}
		else {
			result = Expressions.make(RuleConverter.FUNCTOR_CONDITIONAL_RULE, condition, thenRule);
		}
	...
	public Expression visitComplete_atomic_rule(RuleParser.Complete_atomic_ruleContext ctx) {
		...
		Expression result = Expressions.make(RuleConverter.FUNCTOR_ATOMIC_RULE, formula, potential);
	...
	public Expression visitFormulaNot(RuleParser.FormulaNotContext ctx) {
		Expression result = Expressions.make(Not.FUNCTOR, visit(ctx.formula()));
		return result;
	}
	...
	public Expression visitFormulaMayBeSameAs(
		...
		Expression result = Expressions.make(RuleConverter.FUNCTOR_MAY_BE_SAME_AS, visit(ctx.leftop), visit(ctx.rightop));
		return result;
	}
